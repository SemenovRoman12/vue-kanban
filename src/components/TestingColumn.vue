<template>
  <div class="column-container">
    ...
    <div v-for="task in tasks" :key="task.id" class="task-card">
      ...
      <button @click="onMoveToDone(task)">Переместить в 'Выполненные'</button>
      <button @click="onReturnTask(task)">Вернуть в 'В работе'</button>
    </div>
    ...
    <div v-if="returningTask" class="modal-backdrop">
      <div class="modal">
        <h3>Причина возврата</h3>
        <textarea v-model="returnReason" placeholder="Опишите причину"></textarea>
...
export default {
  ...
  methods: {
    ...
    confirmReturn() {
      ...
      this.$emit('move-task', {
        id: this.returningTask.id,
        newStatus: 'in-progress',
        reason: this.returnReason
      });
      // Сброс полей
      this.returnReason = '';
      this.returningTask = null;
    },
    // Отмена возврата
    cancelReturn() {
      this.returnReason = '';
